    ...
    def init_repo
      ...
      if (repo_check_http_code != "404")
        ...
        unless @name =~ /^tests_/ then
          ...
          shell_command("delivery init --repo-name #{@name} --github marchex-chef --server delivery.marchex.com --ent marchex --org marchex --skip-build-cookbook --user #{ENV['USER']} --no-open", @name)
